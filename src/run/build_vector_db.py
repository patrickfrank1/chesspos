...
from src.dataset import token_sequence_to_board
...
if __name__ == "__main__":
    ...
    collection = {"token": None, "embedding": None}
    ...
    for vector in collection.keys():
        if utility.has_collection(f"{COLLECTION}_{vector}"):
            ...
            collection[vector] = Collection(
                f"{COLLECTION}_{vector}", schema[vector], consistency_level="Strong"
            )
    ...
    result = collection["embedding"].search(
        test_embedding, "embedding", search_params, limit=5, output_fields=["pk"]
    )
